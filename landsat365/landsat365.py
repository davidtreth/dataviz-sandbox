...
def createCaption(rawimage, descript, fontsize, left=False, drawBox=True):
    ...
    while (tlength > 0.75*xs):
        # if the width of the text is more than 3/4 the size of the image
...
for p in listpics2:
    ...
    if fullsize:
        ...
        out2 = createCaption(im2, descript, 60)
        # resize to 720px
        xs, ys = im.size
        ys3 = 720
        xs3 = int((ys3/ys) * xs)
        im3 = im.resize((xs3, ys3))
        out3 = createCaption(im3, descript, 48)                
        ...
        outfile3 = os.path.join(outdir, subdir3,mainfilename)
